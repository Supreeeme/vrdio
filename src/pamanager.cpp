...
void PAManager::changeVol(int newPct) {
    ...
    pa_cvolume_set(&(sinkList[defaultSinkIndex].volume), sinkList[defaultSinkIndex].volume.channels, newVol);
    pa_operation *o = pa_context_set_sink_volume_by_index(context, sinkList[defaultSinkIndex].index, &(sinkList[defaultSinkIndex].volume),nullptr,nullptr);
    pa_operation_unref(o);
}

// return volume of the default sink as a percentage (0 - 100)
int PAManager::getVolPct() {
    if (defaultSinkIndex == -1) return 0;
    ...
    pa_operation *o = pa_context_get_sink_info_by_name(context, sinkList[defaultSinkIndex].name.c_str(),
...
void PAManager::changeSink(int sinkIndex) {
    pa_operation *op = pa_context_set_default_sink(context, sinkList[sinkIndex].name.c_str(), nullptr, nullptr);
    pa_operation_unref(op);
    defaultSinkIndex = sinkIndex;
    emit newDefaultSink();
}

int PAManager::getDefaultSinkIndex() const {
    return defaultSinkIndex;
}
...
void PAManager::changeCardProfile(Card *card, const QString& profileName) {
    ...
    pa_operation *o = pa_context_set_card_profile_by_index(context, card->index, profile_name.c_str(), callback, mainloop);
    pa_threaded_mainloop_unlock(mainloop);
    ...
    // PipeWire likes to change the sink after changing a card - switch back to the current default sink,
    // if it's still available. Otherwise, just guess which sink the user wants
    pa_threaded_mainloop_lock(mainloop);
    ...
    if (!oldSinkExists){
        ...
        // some profile names seem to have two colons? the stuff after the last one seems to be the sink name
...
void PAManager::loadConfig() {
    ...
    auto callback = [](pa_context *c, int success, void* data){
        auto items = static_cast<cbStruct*>(data);
        if (!success)
            std::clog << "Could not set profile " << items->profileStr.toStdString() << " on card " << items->cardStr.toStdString() << std::endl;
        pa_threaded_mainloop_signal(items->mainloop, 0);
    };

    pa_operation* o = pa_context_set_card_profile_by_name(context, card.toUtf8(), profile.toUtf8(), callback, &s);
    pa_threaded_mainloop_unlock(mainloop);
    waitForOpFinish(o);
